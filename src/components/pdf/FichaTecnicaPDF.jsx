...
// ─── Page geometry ─────────────────────────────────────────────────────────────
// These MUST mirror the values used later when the PDF is actually assembled
// (see FOOTER_H / TOP_M inside the useEffect below). Keeping a single source
// of truth here is what lets avoidPageCuts() predict the real page breaks
// instead of assuming every page is a full, margin-free 297mm tall.
const A4_W_MM    = 210
const A4_H_MM    = 297
const FOOTER_MM  = 9    // footer band reserved at the bottom of every page
const TOPM_MM    = 12   // blank margin reserved at the top of pages 2+
const MM_TO_PX   = W / A4_W_MM                      // css px per mm in the template's own coordinate space
const FOOTER_PX  = FOOTER_MM * MM_TO_PX
const TOPM_PX    = TOPM_MM   * MM_TO_PX
const PAGE1_PX   = PAGE_H - FOOTER_PX               // visible height of page 1 (no top margin, has footer)
const PAGEN_PX   = PAGE_H - FOOTER_PX - TOPM_PX      // visible height of every page after that (has both)

// Given a y-position (template px), returns the y-position where the page
// that contains it actually ends (i.e. where content would get sliced off).
...
// ─── Page-cut avoidance (absolute-offset aware) ───────────────────────────────
function avoidPageCuts(template) {
  ...
  for (let iter = 0; iter < 40; iter++) {
    ...
    for (const s of sections) {
      ...
      if (h > PAGEN_PX) continue // taller than a full page — pushing it won't help, let it flow
    ...
    <div style={{ position:'fixed', inset:0, zIndex:9999, background:'rgba(15,23,42,0.65)', display:'flex', alignItems:'center', justifyContent:'center', backdropFilter:'blur(8px)' }}>
      <div style={{ background:'#fff', borderRadius:24, padding:'32px 44px', textAlign:'center', minWidth:300, boxShadow:'0 32px 64px rgba(0,0,0,0.2)' }}>
        {status === 'error' ? (
          ...
          <div style={{ fontFamily:SERIF, fontSize:20, fontWeight:600, color:SL[800] }}>¡Ficha descargada!</div>
        ) : (
          <>
            <div style={{ fontFamily:SERIF, fontSize:20, fontWeight:600, color:SL[800], marginBottom:5 }}>Generando ficha técnica...</div>
            <div style={{ fontFamily:SANS, fontSize:11.5, color:SL[500], marginBottom:20 }}>Compilando datos, imágenes y diseño</div>
    ...
    <div style={{ width:W, background:'#fff', fontFamily:SANS, color:SL[900] }}>
      ...
      <div style={{ padding:`0 ${PAD}px` }}>
        ...
        <section data-section="kpis" style={{ marginBottom:22 }}>
          <SecLabel>Indicadores de seguimiento</SecLabel>
        ...
        <section data-section="finca" style={{ marginBottom:22 }}>
          <SecLabel>Finca y parcela</SecLabel>
        ...
        <section data-section="contexto" style={{ marginBottom:22 }}>
          <SecLabel>Contexto clínico</SecLabel>
        ...
        {sevDistEntries.length > 0 && (
          <section data-section="severidad" style={{ marginBottom:22 }}>
            <SecLabel>Distribución de severidad</SecLabel>
        ...
        {fichaDiseases.length > 0 && (
          <section data-section="diagnosticos" style={{ marginBottom:22 }}>
            <SecLabel>Diagnósticos identificados</SecLabel>
            <div style={{ border:`1px solid ${SL[200]}`, borderRadius:11, overflow:'hidden' }}>
        ...
        {imagesWithUrl.length > 0 && (
          <section data-section="galeria" style={{ marginBottom:22 }}>
            <SecLabel>Registro fotográfico</SecLabel>
